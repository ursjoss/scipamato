...
public class NewStudyItemWriter extends ScipamatoItemWriter<PublicNewStudy> {
    ...
    protected int executeUpdate(final PublicNewStudy p) {
        return getDslContext()
            ...
            .columns(NEW_STUDY.NEWSLETTER_ID, NEW_STUDY.NEWSLETTER_TOPIC_ID, NEW_STUDY.SORT, NEW_STUDY.PAPER_NUMBER, NEW_STUDY.YEAR, NEW_STUDY.AUTHORS, NEW_STUDY.HEADLINE, NEW_STUDY.DESCRIPTION, NEW_STUDY.VERSION, NEW_STUDY.CREATED,
                NEW_STUDY.LAST_MODIFIED, NEW_STUDY.LAST_SYNCHED)
            .values(p.getNewsletterId(), p.getNewsletterTopicId(), p.getSort(), p.getPaperNumber(), p.getYear(), p.getAuthors(), p.getHeadline(), p.getDescription(),  p.getVersion(), p.getCreated(), p.getLastModified(),
                p.getLastSynched())
            ...
            .where(NEW_STUDY.NEWSLETTER_ID.eq(p.getNewsletterId()).and(NEW_STUDY.PAPER_NUMBER.eq(p.getPaperNumber())))
            .execute();
    }
