...
internal class NewsletterUpdateSetStepSetterTest : UpdateSetStepSetterTest<NewsletterRecord, Newsletter>() {
    ...
    override fun verifyCallToAllFieldsExceptAudit() {
        verify<Newsletter>(entityMock).issue
        verify<Newsletter>(entityMock).issueDate
        verify<Newsletter>(entityMock).publicationStatus
    }

    override fun verifyStepSettingExceptAudit() {
        verify(step).set(NEWSLETTER.ISSUE, ISSUE)
        verify<UpdateSetMoreStep<NewsletterRecord>>(moreStep).set(NEWSLETTER.ISSUE_DATE, Date.valueOf(ISSUE_DATE))
        verify<UpdateSetMoreStep<NewsletterRecord>>(moreStep).set(NEWSLETTER.PUBLICATION_STATUS, NewsletterRecordMapperTest.PUBLICATION_STATUS.id)
    }

    override fun verifyStepSettingAudit() {
        verify<UpdateSetMoreStep<NewsletterRecord>>(moreStep).set(NEWSLETTER.CREATED, RecordMapperTest.CREATED)
        verify<UpdateSetMoreStep<NewsletterRecord>>(moreStep).set(NEWSLETTER.CREATED_BY, RecordMapperTest.CREATED_BY)
        verify<UpdateSetMoreStep<NewsletterRecord>>(moreStep).set(NEWSLETTER.LAST_MODIFIED, RecordMapperTest.LAST_MOD)
        verify<UpdateSetMoreStep<NewsletterRecord>>(moreStep).set(NEWSLETTER.LAST_MODIFIED_BY, RecordMapperTest.LAST_MOD_BY)
        verify<UpdateSetMoreStep<NewsletterRecord>>(moreStep).set(NEWSLETTER.VERSION, RecordMapperTest.VERSION + 1)
    }
