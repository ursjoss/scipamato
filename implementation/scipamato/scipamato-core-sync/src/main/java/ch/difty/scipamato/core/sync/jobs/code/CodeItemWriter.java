...
/**
 * {@link ItemWriter} implementation to synchronize codes from scipamato-core to public.
 *
 * <ul>
...
public class CodeItemWriter extends ScipamatoItemWriter<PublicCode> {
    ...
    public CodeItemWriter(final DSLContext jooqDslContextPublic) {
        super(jooqDslContextPublic, "code");
    }

    @Override
    protected int executeUpdate(final PublicCode c) {
        return getDslContext()
            .insertInto(CODE)
            .columns(CODE.CODE_, CODE.LANG_CODE, CODE.CODE_CLASS_ID, CODE.NAME, CODE.COMMENT, CODE.SORT, CODE.VERSION, CODE.CREATED, CODE.LAST_MODIFIED, CODE.LAST_SYNCHED)
            .values(c.getCode(), c.getLangCode(), c.getCodeClassId(), c.getName(), c.getComment(), c.getSort(), c.getVersion(), c.getCreated(), c.getLastModified(), c.getLastSynched())
            ...
            .where(CODE.CODE_.eq(c.getCode()).and(CODE.LANG_CODE.eq(c.getLangCode())))
            .execute();
    }
