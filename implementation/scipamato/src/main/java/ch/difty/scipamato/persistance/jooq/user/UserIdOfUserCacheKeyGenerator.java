...
/**
 * CacheKeyGenerator extracting the user id from the {@link User} object as key.
 *
 * @author u.joss
 */
public class UserIdOfUserCacheKeyGenerator implements CacheKeyGenerator {
    ...
    public GeneratedCacheKey generateCacheKey(final CacheKeyInvocationContext<? extends Annotation> ctx) {
        ...
        for (final CacheInvocationParameter parameter : allParameters) {
            if (User.class.equals(parameter.getRawType())) {
                ...
                return new DefaultGeneratedCacheKey(new Object[] { user.getId() });
            }
        }
