 ...
 * 
 * We need this workaround in order to access the audit fields from the record, a class which is generated, so we can't
 * simply implement an interface that offers the audit methods for the record. Hence the dto AuditFields class that needs
 * to provide those fields from the concrete class.
 *
 * @author u.joss
 *
 * @param <R> the type of the {@link Record}
 * @param <T> the type of the {@link SipamatoEntity}
 */
public abstract class EntityRecordMapper<R extends Record, T extends SipamatoEntity> implements RecordMapper<R, T> {
    ...
    /**
     * Implement the mapping of the normal (i.e. non-audit) fields from record into entity.
     *
     * @param from the record to provide the fields from db
     * @param to the entity to fill the fields into
     */
     ...
     *
     * @param from the record to provide the audit fields from db
     * @param to the entity to fill the audit fields into
     */
