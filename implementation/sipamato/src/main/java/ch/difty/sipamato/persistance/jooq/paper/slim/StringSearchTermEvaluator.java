...
class StringSearchTermEvaluator implements SearchTermEvaluator<StringSearchTerm> {
    ...
    public Condition evaluate(final StringSearchTerm searchTerm) {
        ...
        case EXACT:
            return DSL.field(searchTerm.getFieldName()).lower().equal(DSL.val(searchTerm.getValue()).lower());
        case CONTAINS:
            return DSL.field(searchTerm.getFieldName()).lower().contains(DSL.val(searchTerm.getValue()).lower());
        case STARTS_WITH:
            return DSL.field(searchTerm.getFieldName()).lower().startsWith(DSL.val(searchTerm.getValue()).lower());
        case ENDS_WITH:
            return DSL.field(searchTerm.getFieldName()).lower().endsWith(DSL.val(searchTerm.getValue()).lower());
        default:
