...
open class NewStudyListPage(parameters: PageParameters) : BasePage<Void>(parameters) {
    ...
    private fun newNewStudyCollection(id: String): ListView<NewStudyTopic> {
        ...
        return object : ListView<NewStudyTopic>(id, topics) {
            override fun populateItem(topic: ListItem<NewStudyTopic>) {
                topic.add(Label("topicTitle", PropertyModel<Any>(topic.model, NewStudyTopic.NewStudyTopicFields.TITLE.fieldName)))
                topic.add(object : ListView<NewStudy>("topicStudies", topic.modelObject.studies) {
                    override fun populateItem(study: ListItem<NewStudy>) {
                        study.add(Label("headline", PropertyModel<Any>(study.model, NewStudy.NewStudyFields.HEADLINE.fieldName)))
                        study.add(Label("description", PropertyModel<Any>(study.model, NewStudy.NewStudyFields.DESCRIPTION.fieldName)))
    ...
    private fun retrieveStudyCollection(): List<NewStudyTopic> {
        val issue = pageParameters[PublicPageParameters.ISSUE.getName()]
     ...
     *
     * @param id the id on the html page. Also expects a label with the same id + tag 'Label'
     * @param study the current study as ListItem
     * @return the link
     */
    private fun newLinkToStudy(id: String, study: ListItem<NewStudy>): Link<NewStudy> {
        val pp = PageParameters()
        pp[PublicPageParameters.NUMBER.getName()] = study.modelObject.number
        return object : Link<NewStudy>(id) {
            ...
            add(Label("${id}Label", PropertyModel<Any>(study.model, NewStudy.NewStudyFields.REFERENCE.fieldName)))
        }
    }
    ...
    private fun newExternalLink(id: String, linkItem: ListItem<NewStudyPageLink>) = object :
        BootstrapExternalLink(id, Model.of(linkItem.modelObject.url)) {}.apply<BootstrapExternalLink> {
    ...
    private fun newLabel(id: String): Label = Label(id, StringResourceModel("$id$LABEL_RESOURCE_TAG", this, null).string)
    ...
    private fun newLinkToArchivedNewsletter(id: String, newsletter: ListItem<Newsletter>): Link<Newsletter> {
        val pp = PageParameters()
        pp[PublicPageParameters.ISSUE.getName()] = newsletter.modelObject.issue
        ...
        return object : BootstrapLink<Newsletter>(id, Buttons.Type.Link) {
            override fun onClick() {
                setResponsePage(NewStudyListPage(pp))
            }
        }.apply {
