...
class PublicPage(parameters: PageParameters) : BasePage<Void>(parameters) {
    ...
    private inner class TabPanel2(
        ...
        private fun makeCodeClassComplex(
            ...
            val choiceRenderer = ChoiceRenderer<Code>(Code.CodeFields.DISPLAY_VALUE.fieldName, Code.CodeFields.CODE.fieldName)
    ...
    private fun makeTableColumns() = mutableListOf<IColumn<PublicPaper, String>>().apply {
        add(makePropertyColumn(PublicPaper.PublicPaperFields.AUTHORS_ABBREVIATED, PublicPaper.PublicPaperFields.AUTHORS))
        add(makeClickableColumn(PublicPaper.PublicPaperFields.TITLE, this@PublicPage::onTitleClick))
        add(makePropertyColumn(PublicPaper.PublicPaperFields.JOURNAL, PublicPaper.PublicPaperFields.LOCATION))
        add(makePropertyColumn(PublicPaper.PublicPaperFields.PUBL_YEAR, PublicPaper.PublicPaperFields.PUBL_YEAR))
    }

    private fun makePropertyColumn(fieldType: FieldEnumType, sortField: FieldEnumType) =
        PropertyColumn<PublicPaper, String>(
            StringResourceModel("$COLUMN_HEADER${fieldType.fieldName}", this, null),
            sortField.fieldName,
            fieldType.fieldName
        )

    private fun makeClickableColumn(
        fieldType: FieldEnumType,
        consumer: SerializableConsumer<IModel<PublicPaper>>,
    ): ClickablePropertyColumn<PublicPaper, String> {
        val propExpression = fieldType.fieldName
        return ClickablePropertyColumn(StringResourceModel(COLUMN_HEADER + propExpression, this, null), propExpression, consumer,
            propExpression, true)
    }
